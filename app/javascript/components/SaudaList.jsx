...
// What the seller's register comes to. A sauda that carries no figure yet
...
export default function SaudaList({ saudas, loading, sellerName, onEdit, onDelete, deletingId }) {
  ...
  return (
    <>
      {/* What the register adds up to, before the rows it is made of. */}
      <dl className="totals">
        <div className="totals__item">
          <dt>Total tax bill amt</dt>
          <dd>{money(totalOf(saudas, "total_tax_bill_amt"))}</dd>
        </div>
        <div className="totals__item">
          <dt>Total amount</dt>
          <dd>{money(totalOf(saudas, "amount"))}</dd>
        </div>
        <div className="totals__item">
